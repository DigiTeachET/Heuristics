// initialising variables for score
var _score = 0;
var _answered = 0;
var total = 20;

// Setting up lists for the heuristics
var heuristics = [
...
// definition list of the heuristics
var heuristicDefs = {
    "1. Visibility of system status": "The system should always keep users informed about what is going on, through appropriate feedback within reasonable time. Tell people what is happening, or they will assume the worst — and then click Submit eleven more times.",
    "2. Match with real world": "The system should speak the users' language — words, phrases, and concepts familiar to the user rather than system-oriented terms. Follow real-world conventions so information appears in a natural and logical order.",
    "3. User control and freedom": "Users often choose system functions by mistake. They need a clearly marked emergency exit to leave the unwanted state without going through an extended ordeal. Support undo and redo — because humans are fallible, and that is fine.",
    "4. Consistency and standards": "Users should not have to wonder whether different words, situations, or actions mean the same thing. Follow platform and industry conventions so the interface behaves predictably every single time.",
    "5. Error prevention": "Even better than a helpful error message is a design that stops the mistake happening in the first place. Eliminate error-prone conditions, or check for them and present a confirmation step before the user commits to something irreversible.",
    "6. Recognition not recall": "Minimise the user's memory load by making objects, actions, and options visible. The user should not have to remember information from one part of an interface to another — the interface should do that job.",
    "7. Flexibility and efficiency": "Accelerators — hidden from the novice user — may speed up interaction for the expert, so the system can serve both. Allow users to tailor frequent actions.",
    "8. Aesthetic and minimalist design": "Dialogues should not contain irrelevant or rarely needed information. Every extra unit of information competes with the relevant information and reduces its relative visibility. The cheese videos were not helping anyone.",
    "9. Help users recognize, diagnose, and recover from errors": "Error messages should be expressed in plain language (no error codes), precisely indicate the problem, and constructively suggest a solution. 'XMIT_ERR_0x4F2' does not count.",
    "10. Help and documentation": "Even though it is better if the system can be used without help, when help is needed it should be easy to search, focused on the user's task, and consist of concrete steps — not a loop that links back to itself."
  };

//Various scenarios - using sarcasm and some humour - with the list of questions and the correct answer - along with feedback
var scenarios = [
    {
...
//Combine both sets into a single array. All game logic reads from allScenarios only. To extend the quiz: concat additional scenario arrays here and increment the total variable at the top of the script.
var allScenarios = scenarios.concat(scenariosB);

//GAME LOGIC
//SHUFFLE Fisher-Yates algorithm: produces a randomly ordered copy of an array without modifying the original. Called on both the scenario order and button order
...
//HANDLE ANSWER Called by the onclick of every heuristic button. Parameters: chosen  the label text of the clicked button btn  the DOM button element itself 
function handleAnswer(chosen, btn) {
  ...
  //Disable all buttons so the student cannot change their answer
    ...
    if (isCorrect) {
      //Correct answer: green button, success feedback message
      ...
      //Definition block appended below the explanation
        ...
        '<span style="font-size:12px; text-transform:uppercase; letter-spacing:0.05em; opacity:0.7;">Definition — ' + s.correctLabel + '</span>' +
...
//NEXT SCENARIOCalled by the Next / See results button. Either moves to the next question or triggers the end screen.
...
// Builds the HTML for one scenario card and writes it intothe #scenarios div. Scrolls to the top so the student does not have to scroll up to read the new question. 
function renderCurrent() {
    var i = _order[_current];
    var s = allScenarios[i];
    var container = document.getElementById('scenarios');
    //Shuffle button order freshly for each scenari
    var shuffled = shuffle(heuristics);
    var btnsHtml = shuffled.map(function(h) {
      //Escape single quotes in label text to avoid breaking the inline onclick
      return '<button class="heuristic-btn" onclick="handleAnswer(\'' + h.replace(/'/g, "\\'") + '\', this)">' + h + '</button>';
    }).join('');
    //Replace the scenario slot with the new card 
    container.innerHTML =
      '<div class="scenario-card" id="sc-current">' +
      //Progress counter
      '<p style="font-size:12px; color:#888780; margin-bottom:6px;">Scenario ' + (_current + 1) + ' of ' + total + '</p>' +
      '<h2>' + s.title + '</h2>' +
      '<p>' + s.desc + '</p>' +
      //Answer buttons
      ...
      '<button id="next-btn" class="reset-btn" style="display:none;" onclick="nextScenario()">Next scenario</button>' +
      '</div>' +
      '</div>';
...
//Clears the scenario slot, reveals the end score card,picks an appropriate te reo message based on percentage, and shows the confusion comparison card.
...
// Reset activity - re-shuffles scenario order, hides the end cards, and renders the first question again. - Called by the try again button on the finished card
function resetActivity() {
    _score = 0;
    _answered = 0;
    _current = 0;
// reshuffle so students get a differnt order on ech attempt
...
// initialisation - runs once the page loads and creates the inital shuffled order
  _order = shuffle(allScenarios.map(function(_, i) { return i; }));
  renderCurrent();
